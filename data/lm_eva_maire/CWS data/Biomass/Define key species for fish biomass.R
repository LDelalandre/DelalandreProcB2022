...
#                                                                                                              #
# Here we apply this framework to define the "key fish species" for total fish biomass                         # 
...
fish_sp <- colnames(presence_fish_species)

occurrence <- apply(presence_fish_species,2,sum)
...
presence_candidates <- presence_fish_species[,required_threshold]

#Verifying that the 2 datasets have the same rows
length(is.element(Biomass_data$Reef_ID,rownames(presence_candidates))) # 1824 OK
...
M0 <- lmer(logBiomass~Management+Gravity_markets+Gravity_near_pop+Species_richness+Local_population_growth+
             HDI+Voice_accountability+Tourism+Population_size+Reef_fish_landings+Oceanic_productivity+Depth+
             Habitat+Census_method+Total_sampling_area+
             (1|Nation)+(1|Site)+(1|Regional_location),data=Biomass_data)

#Checking for robustness of M0
AIC(M0)
r.squaredGLMM(M0)[2] #Radj = 0.79

#Storing AIC of M0 that will be the threshold to define which species improve the prediction of fish biomass
AIC_M0 <- AIC(M0)
...
for (k in 1:length(candidates_species)) {
  ...
  data_with_candidate <-as.data.frame(cbind(Biomass_data,candidate)) 
  
  #Adding presence/absence of species k to M0 to obtain M1
  M1 <- lmer(logBiomass~candidate+Management+Gravity_markets+Gravity_near_pop+Species_richness+Local_population_growth+HDI+Voice_accountability+Tourism+
               Population_size+Reef_fish_landings+Oceanic_productivity+Depth+Habitat+Census_method+Total_sampling_area+
...
#Determining species that improve the prediction of fish biomass as ΔAIC (AIC_M0 - AIC_M1) > 4 with a positive effect
