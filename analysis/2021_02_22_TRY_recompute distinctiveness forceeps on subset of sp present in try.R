...
bind_forceeps <- function (dist.ForCEEPS,dist.focus) { # group distinctiveness output of forceeps and another dataset
  ...
  toplot <- data.frame(dist.forceeps %>% arrange(SName) %>% pull(SName),
                       dist.forceeps %>% arrange(SName) %>% pull(Di),
                       dist.focus %>% arrange(SName) %>% pull(Di))
  colnames(toplot) <- c("SName","Di.forceeps","Di.focus")
  toplot
}

compare_Di <- function(traits.simulations,traitdata){
  ...
  traits.simulations2 <- traits.simulations %>% 
    filter(SName %in% rownames(dist.traitdata0)) %>% 
    select(-c(Name,SName))
...
toplot <- compare_Di(traits.simulations,diaznoSLA)
...
# Compute distinctiveness on these traits and compare it to ForCEEPS
toplot <- compare_Di(traits.simulations,pro)
cortest <- cor.test(toplot$Di.forceeps,toplot$Di.focus,method="spearman")  
